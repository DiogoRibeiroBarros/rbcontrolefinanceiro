...
export const MOBILE_VERSION = '1.1.14';
...
export async function findLatestMobileRelease(fetcher: typeof fetch = fetch): Promise<MobileRelease | null> {
  ...
  return releases.filter(item => isNewerMobileRelease(item.tag_name) && item.assets.some(asset => /\.apk$/i.test(asset.name)))
