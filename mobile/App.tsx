...
function AppContent() {
  ...
  useEffect(() => {
    ...
    void findLatestMobileRelease(String(Application.nativeApplicationVersion || MOBILE_VERSION)).then(release => {
      ...
      Alert.alert('Atualização disponível', `${release.name || release.tag_name}\n\n${release.body || 'Uma nova versão está pronta.'}`, [
        { text:'Atualizar agora', onPress:() => { void downloadAndInstall(asset.browser_download_url, asset.name); } }
      ]);
    }).catch(() => undefined);
  ...
  const downloadAndInstall = async (url:string, name:string) => {
    try {
      ...
      await IntentLauncher.startActivityAsync('android.intent.action.VIEW', { data:contentUri, type:'application/vnd.android.package-archive', flags:1 | 2 });
    } catch {
...
function ConnectionScreen({ initial, busy, onSave }: { initial:SyncConfiguration; busy:boolean; onSave:(value:SyncConfiguration, code:string, name:string)=>void }) {
  const [baseUrl, setBaseUrl] = useState(initial.baseUrl); const [code, setCode] = useState(''); const [name, setName] = useState('Meu celular'); const [step, setStep] = useState(1);
  const next = () => { if (!baseUrl.trim()) { Alert.alert('Endereço obrigatório', 'Informe o endereço HTTPS exibido em Configurações → Sistema.'); return; } setStep(2); };
  const submit = () => { const url = baseUrl.trim(); const digits = code.replace(/\D/g, '').slice(0, 6); if (!/^\d{6}$/.test(digits)) { Alert.alert('Código inválido', 'Informe os seis dígitos exibidos no desktop.'); return; } onSave({ baseUrl:url, accessToken:'' }, digits, name.trim() || 'Meu celular'); };
  return <SafeAreaView style={styles.connection} edges={['top','right','bottom','left']}><StatusBar style="light" backgroundColor="#0d0e10" /><Image source={require('./assets/rb_gestao_horizontal_sidebar_hd.png')} style={styles.connectionLogo} resizeMode="contain" /><View style={styles.connectionCard}><Text style={styles.title}>Conectar ao desktop</Text><Text style={styles.muted}>Etapa {step} de 3</Text>{step === 1 ? <><Text style={styles.muted}>Informe o endereço HTTPS exibido em Configurações → Sistema.</Text><Text style={styles.label}>URL do desktop</Text><TextInput autoCapitalize="none" autoCorrect={false} keyboardType="url" value={baseUrl} onChangeText={setBaseUrl} placeholder="https://seu-endereco.ts.net" placeholderTextColor="#74777b" style={styles.input} /><Pressable onPress={next} android_ripple={{ color:'#8ac52a' }} style={({ pressed }) => [styles.primary, pressed && styles.primaryPressed]}><Text style={styles.primaryText}>Continuar</Text></Pressable></> : step === 2 ? <><Text style={styles.muted}>Agora informe o código de seis dígitos exibido no desktop.</Text><Text style={styles.label}>Código de conexão</Text><TextInput autoFocus autoCapitalize="none" autoCorrect={false} keyboardType="number-pad" maxLength={6} value={code} onChangeText={value => setCode(value.replace(/\D/g,'').slice(0, 6))} placeholder="000000" placeholderTextColor="#74777b" style={styles.input} /><View style={styles.stepActions}><Pressable onPress={() => setStep(1)} style={styles.secondary}><Text style={styles.secondaryText}>Voltar</Text></Pressable><Pressable onPress={() => /^\d{6}$/.test(code) ? setStep(3) : Alert.alert('Código inválido', 'Informe os seis dígitos exibidos no desktop.')} style={styles.primarySmall}><Text style={styles.primaryText}>Continuar</Text></Pressable></View></> : <><Text style={styles.muted}>Escolha um nome para identificar este dispositivo no desktop.</Text><Text style={styles.label}>Nome deste dispositivo</Text><TextInput autoFocus value={name} onChangeText={setName} placeholder="Meu celular" placeholderTextColor="#74777b" style={styles.input} /><View style={styles.stepActions}><Pressable onPress={() => setStep(2)} style={styles.secondary}><Text style={styles.secondaryText}>Voltar</Text></Pressable><Pressable disabled={busy} onPress={submit} android_ripple={{ color:'#8ac52a' }} style={({ pressed }) => [styles.primarySmall, pressed && styles.primaryPressed, busy && styles.disabled]}>{busy ? <ActivityIndicator color="#11140d" /> : <Text style={styles.primaryText}>Solicitar pareamento</Text>}</Pressable></View></>}</View></SafeAreaView>;
}

const styles = StyleSheet.create({ safe:{ flex:1, backgroundColor:'#0d0e10' }, web:{ flex:1, backgroundColor:'#0d0e10' }, loading:{ flex:1, alignItems:'center', justifyContent:'center', gap:18, backgroundColor:'#0d0e10' }, loadingInline:{ position:'absolute', top:0, right:0, bottom:0, left:0, zIndex:3 }, logo:{ width:220, height:80 }, muted:{ color:'#a7a9ac', lineHeight:21 }, connection:{ flex:1, justifyContent:'center', padding:24, gap:20, backgroundColor:'#0d0e10' }, connectionLogo:{ width:'76%', height:90, alignSelf:'center' }, connectionCard:{ padding:20, gap:12, borderRadius:22, borderWidth:1, borderColor:'#2d3035', backgroundColor:'#1b1d20' }, title:{ color:'#f6f4ee', fontSize:25, fontWeight:'900' }, label:{ color:'#c8cac5', marginTop:6, fontSize:12, fontWeight:'700' }, input:{ height:50, paddingHorizontal:14, borderRadius:13, borderWidth:1, borderColor:'#34373c', backgroundColor:'#282a2e', color:'#f6f4ee', fontSize:15 }, primary:{ height:52, marginTop:8, alignItems:'center', justifyContent:'center', borderRadius:13, backgroundColor:'#b7ff3c', overflow:'hidden' }, primarySmall:{ flex:1, height:52, alignItems:'center', justifyContent:'center', borderRadius:13, backgroundColor:'#b7ff3c', overflow:'hidden' }, secondary:{ flex:1, height:52, alignItems:'center', justifyContent:'center', borderRadius:13, backgroundColor:'#2a2c31' }, secondaryText:{ color:'#f6f4ee', fontWeight:'800' }, stepActions:{ flexDirection:'row', gap:10, marginTop:8 }, primaryPressed:{ opacity:.75, transform:[{ scale:.98 }] }, disabled:{ opacity:.45 }, primaryText:{ color:'#11140d', fontWeight:'900', fontSize:15 }, errorBar:{ paddingHorizontal:12, paddingVertical:8, flexDirection:'row', alignItems:'center', gap:10, backgroundColor:'#4a2228' }, errorText:{ flex:1, color:'#ffe8ea', fontSize:12 }, retry:{ color:'#b7ff3c', fontWeight:'800' }, configure:{ color:'#fff', fontWeight:'800' } });
