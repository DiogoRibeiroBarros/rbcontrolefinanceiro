...
['renderRemoteAccessSettings','rotatePairingCode','copyRemoteAccessLink','pairingCode','remote-access-link'].forEach((feature) => assert.ok(appSource.includes(feature)));
assert.ok(!appSource.includes('id="remote-public-url"'), 'O endereço não deve depender de digitação manual.');
assert.ok(!css.includes('.settings-section:nth-child('), 'Abas não podem depender da posição do card.');
['renderUpdateSettings','renderAppearanceSettings','renderBackupSettings','switch (settingsTab)'].forEach((feature) => assert.ok(appSource.includes(feature)));
...
['sync:status','sync:refresh','sync:rotate-code','publicUrl','accessUrl','SYNC_ACCESS_TOKEN','mobile?key='].forEach((feature) => assert.ok(electronMain.includes(feature)));
...
['sync:status','sync:refresh','sync:rotate-code','updates:status','updates:prompt','updates:download','updates:install'].forEach((feature) => assert.ok(preloadSource.includes(feature)));
