...
(async () => { try {
  ...
  const pending = restored.requestPairing({ code, name:'Segundo celular', clientId:'client-b' });
  assert.equal(restored.listPending()[0].name, 'Segundo celular');
  restored.decide(pending.requestId, false);
  assert.equal(restored.listPending().length, 0);
  restored.setDeviceStatus(result.deviceId, 'blocked');
  assert.equal(restored.getStatus().pairedDevices, 0);
  assert.equal(restored.authenticate(result.token), null);
  restored.setDeviceStatus(result.deviceId, 'active');
  assert.ok(restored.authenticate(result.token));
