...
function Setup({pair,busy,message,pending}:{pair:(url:string,code:string,name:string)=>Promise<void>;busy:boolean;message:string;pending:boolean}) {
  ...
  if(pending) return <View style={styles.card}><Text style={styles.title}>Aguardando aprovação</Text><Text style={styles.body}>Confirme a solicitação deste celular na tela Configurações do RB Gestão no PC.</Text><Text style={styles.body}>O vínculo será concluído aqui automaticamente.</Text>{message ? <Text style={styles.error}>{message}</Text>:null}</View>;
  return <View style={styles.card}><Image source={require('./assets/android-icon-foreground.png')} style={styles.logo} resizeMode="contain"/><Text style={styles.title}>Conectar ao desktop</Text><Text style={styles.body}>{['1. Informe o link de acesso mostrado no PC.','2. Digite o código de seis números.','3. Dê um nome a este celular.'][step]}</Text>
    ...
    <View style={styles.row}>{step>0?<Button title="Voltar" secondary onPress={() => setStep(step-1)}/>:null}<Button title={step===2?'Solicitar pareamento':'Continuar'} disabled={busy|| (step===0&&!url.trim()) || (step===1&&!/^\d{6}$/.test(code))} onPress={() => {if(step<2)setStep(step+1);else void pair(url,code,name.trim()||'Meu celular');}}/></View>
  </View>;
}
...
const styles=StyleSheet.create({root:{flex:1,backgroundColor:'#111213'},top:{padding:16,flexDirection:'row',justifyContent:'space-between',alignItems:'center',backgroundColor:'#1b1c1d'},brand:{color:green,fontSize:18,fontWeight:'900'},status:{color:'#eee',fontSize:12},setup:{flexGrow:1,justifyContent:'center'},card:{margin:16,padding:20,backgroundColor:panel,borderRadius:18,gap:14},logo:{width:130,height:105,alignSelf:'center'},title:{color:'#fff',fontSize:23,fontWeight:'800'},body:{color:muted,fontSize:15,lineHeight:22},error:{color:'#ff9999',fontSize:14},input:{backgroundColor:'#292a2c',borderColor:'#414245',borderWidth:1,borderRadius:12,padding:14,color:'#fff',fontSize:17},row:{flexDirection:'row',flexWrap:'wrap',gap:8,alignItems:'center'},button:{backgroundColor:green,paddingHorizontal:16,paddingVertical:12,borderRadius:12},secondary:{backgroundColor:'#303134'},disabled:{opacity:0.4},buttonText:{fontWeight:'800',color:'#111'},secondaryText:{color:'#fff'},web:{flex:1,backgroundColor:'#111'},footer:{flexDirection:'row',justifyContent:'space-between',padding:10,backgroundColor:'#1b1c1d'},footerText:{color:'#aaa',fontSize:11},full:{flex:1,padding:16},log:{borderBottomColor:'#333',borderBottomWidth:1,paddingVertical:10},overlay:{flex:1,justifyContent:'center',backgroundColor:'#000b'}});
