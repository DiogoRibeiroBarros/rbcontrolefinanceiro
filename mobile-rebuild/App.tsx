...
export default function App() {
  ...
  return <SafeAreaView style={styles.root}><StatusBar style="light"/>
    ...
    {!mobile.loaded?<View style={styles.card}><Text style={styles.body}>Carregando vínculo salvo...</Text></View>:!device?<ScrollView contentContainerStyle={styles.setup}><Setup pair={mobile.pair} busy={mobile.busy} message={mobile.message} pending={Boolean(mobile.pending)}/></ScrollView>:<>
      {!ready || !showWeb?<View style={styles.card}><Text style={styles.title}>{!showWeb&&offline?'Últimos dados salvos':device.name}</Text><Text style={styles.body}>PC: {device.origin}</Text><Text style={styles.body}>{labels[mobile.status]}</Text>{mobile.message?<Text style={styles.error}>{mobile.message}</Text>:null}{!showWeb&&offline?<View style={styles.offlineBox}><Text style={styles.offlineTitle}>Modo offline</Text><Text style={styles.body}>Sincronizado em {new Date(offline.savedAt).toLocaleString('pt-BR')}</Text><Text style={styles.body}>Perfis: {cachedProfiles.length} · Lançamentos: {Array.isArray(cachedData.entries)?cachedData.entries.length:0}</Text><Text style={styles.body}>Contas: {Array.isArray(cachedData.bankAccounts)?cachedData.bankAccounts.length:0} · Cartões: {Array.isArray(cachedData.cards)?cachedData.cards.length:0}</Text><Button title="Tentar conexão" onPress={()=>{void mobile.heartbeat();setWebKey(value=>value+1);setShowWeb(true);}} /></View>:offline?<View style={styles.offlineBox}><Text style={styles.offlineTitle}>Dados salvos disponíveis</Text><Text style={styles.body}>Última sincronização: {new Date(offline.savedAt).toLocaleString('pt-BR')}</Text><Text style={styles.body}>Você pode consultar o último estado salvo mesmo sem conexão com o PC.</Text><Button title="Ver dados salvos" onPress={()=>setShowWeb(false)} /></View>:null}
