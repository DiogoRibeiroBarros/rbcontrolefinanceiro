(function (root) {
  ...
  function homeShares(bill) {
    ...
    var cents = Math.round(homeBillAmount(bill) * 100), base = Math.floor(cents / ids.length), remainder = cents % ids.length, confirmations=homeBillConfirmations(bill);
    return ids.map(function(id,index){ return { residentId:id, amount:(base + (index < remainder ? 1 : 0)) / 100, confirmed:Boolean(confirmations[id]) }; });
  }
  ...
  function homeSummary() {
    ...
    homeExpensesData().bills.filter(function(bill){ return homeBillType(bill)==='Fixa mensal' ? selectedMonth >= (bill.startMonth||monthFromBR(bill.dueDate)) : monthFromBR(bill.dueDate)===selectedMonth; }).forEach(function(bill){ homeShares(bill).forEach(function(share){ if(!result[share.residentId])return; result[share.residentId].expected += share.amount; if(share.confirmed)result[share.residentId].confirmed += share.amount; });var payerId=homeBillPayerId(bill);if(result[payerId]) result[payerId].paid += homeBillAmount(bill); });
    homeExpensesData().residentDebts.forEach(function(debt){var started=monthFromBR(debt.date)<=selectedMonth,settledBeforeOrInMonth=debt.status==='Quitada'&&debt.paidDate&&monthFromBR(debt.paidDate)<=selectedMonth;if(!started||settledBeforeOrInMonth)return;var amount=Number(debt.amount||0);if(result[debt.debtorId])result[debt.debtorId].debtOwed+=amount;if(result[debt.creditorId])result[debt.creditorId].debtReceivable+=amount;});
  ...
  function renderHomeExpenses() {
    ...
    var billCards=bills.length?bills.map(function(bill){var shares=homeShares(bill), payer=homeResident(homeBillPayerId(bill)), confirmed=shares.filter(function(item){return item.confirmed;}).length,currentAmount=homeBillAmount(bill);return '<div class="card home-bill-card"><div class="card-header"><div><div class="card-title">'+escapeHtml(bill.title)+'</div><div class="card-subtitle"><span class="pill '+(homeBillType(bill)==='Fixa mensal'?'green':'yellow')+'">'+homeBillType(bill)+'</span> '+escapeHtml(bill.category)+' · competência '+escapeHtml(selectedMonth)+' · vence em '+escapeHtml(homeBillDueDate(bill))+' · pago por '+escapeHtml(payer?payer.name:'Não informado')+'</div></div><strong class="yellow">'+money(currentAmount)+'</strong></div><div class="home-shares">'+shares.map(function(share){var resident=homeResident(share.residentId);return '<div class="home-share"><span>'+escapeHtml(resident?resident.name:'Morador removido')+'</span><strong>'+money(share.amount)+'</strong><button class="'+(share.confirmed?'success-btn':'secondary-btn')+' compact-btn" data-action="toggle-home-payment" data-id="'+bill.id+'" data-resident-id="'+share.residentId+'" '+(!allowed.edit?'disabled':'')+'>'+ (share.confirmed?'Confirmado neste mês':'Confirmar neste mês') +'</button></div>';}).join('')+'</div><div class="row wrap" style="margin-top:12px"><span class="pill '+(confirmed===shares.length&&shares.length?'green':'yellow')+'">'+confirmed+'/'+shares.length+' confirmações em '+escapeHtml(selectedMonth)+'</span><button class="secondary-btn" data-action="home-bill-history" data-id="'+bill.id+'">Histórico mensal</button>'+(allowed.edit?'<button class="secondary-btn" data-action="edit-home-bill" data-id="'+bill.id+'">Editar este mês</button>':'')+(allowed.delete?'<button class="danger-btn" data-action="delete-home-bill" data-id="'+bill.id+'">Excluir</button>':'')+'</div></div>';}).join(''):empty('🏠','Nenhuma conta cadastrada','Cadastre aluguel, energia, internet e demais gastos compartilhados.');
    var debtSummary=residentDebtSummary(),debts=home.residentDebts.slice().sort(function(a,b){if(a.status!==b.status)return a.status==='Pendente'?-1:1;return parseDateBR(b.date)-parseDateBR(a.date);}),debtCards=debts.length?debts.map(function(debt){return renderResidentDebtCard(debt,allowed);}).join(''):empty('🤝','Nenhum acerto entre moradores','Registre aqui quando um morador dever um valor diretamente para outro.');
    var sharedSections='';
  ...
  function openHomeBillForm(bill) {
    ...
    var participants=active.map(function(item){return '<label class="check-option"><input type="checkbox" name="participant" value="'+item.id+'" '+((bill.participantIds||[]).indexOf(item.id)>=0?'checked':'')+'> '+escapeHtml(item.name)+'</label>';}).join('');
    ...
    showModal(editing?'Editar pagamento do mês':'Nova conta da casa','Cada competência guarda seu próprio valor, pagador e confirmações. O histórico dos meses anteriores não será alterado.','<div class="form-grid">'+field('Descrição','title','text',bill.title,'required')+selectField('Categoria','category',['Aluguel','Condomínio','Energia','Água','Internet','Mercado','Gás','Manutenção','Outro'],bill.category)+selectField('Tipo da conta','billingType',['Fixa mensal','Variável'],homeBillType(bill))+'<div class="field"><label for="amount">Valor nesta competência</label><input class="input" id="amount" name="amount" type="text" value="'+escapeHtml(currentAmount?money(currentAmount).replace('R$','').trim():'')+'" placeholder="Ex.: 120,00" required><small class="form-note">Este valor será armazenado somente no mês escolhido.</small></div>'+field('Dia do vencimento','dueDay','number',bill.dueDay||parseDateBR(bill.dueDate).getDate(),'min="1" max="31"')+'<div class="field"><label for="startMonth">'+(editing?'Competência do pagamento':'Mês inicial')+'</label><input class="input" id="startMonth" name="startMonth" type="month" value="'+escapeHtml(targetMonth)+'" required></div>'+'<div class="field"><label>Quem pagou neste mês?</label><select class="select" name="payerId">'+payerOptions+'</select></div><div class="field form-full"><label>Participantes da divisão</label><div class="home-permission-form">'+participants+'</div></div></div>',function(fd){
      ...
      monthlyRecords[recordMonth]={amount:amount,payerId:payerId,confirmations:clone(previousRecord.confirmations||{}),updatedAt:new Date().toISOString()};
      var startMonth=editing?(bill.startMonth||recordMonth):recordMonth;
      var item={id:bill.id||uid(),title:fd.get('title').trim(),category:fd.get('category'),billingType:fd.get('billingType')==='Fixa mensal'?'Fixa mensal':'Variável',amount:amount,dueDate:editing?bill.dueDate:recordDate,dueDay:dueDay,startMonth:startMonth,participantIds:ids,payerId:payerId,confirmations:{},monthlyRecords:monthlyRecords};
