(function (root) {
  ...
  function openHomeDebtForm(debt) {
    ...
    var body='<div class="form-grid">'+selectObjectField('Quem deve','debtorId',options,debt.debtorId)+selectObjectField('Para quem deve','creditorId',options,debt.creditorId)+field('Descrição','description','text',debt.description,'required')+field('Valor devido','amount','text',debt.amount?money(debt.amount).replace('R$','').trim():'','required')+selectField('Tipo da dívida','scheduleType',['Fixa','Programada'],debt.scheduleType||'Fixa')+field('Data do lançamento','date','date',dateInputFromBR(debt.date),'required')+field('Data de vencimento','dueDate','date',dateInputFromBR(debt.dueDate),'required')+textareaField('Observação','notes',debt.notes||'')+'</div><p class="form-note">Dívidas programadas aparecem somente no mês do vencimento. A baixa aparece na competência da data efetiva do pagamento.</p>';
