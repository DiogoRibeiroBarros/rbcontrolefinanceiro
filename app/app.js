(function (root) {
  ...
  function openHomeBillForm(bill) {
    ...
    var participants=active.map(function(item){var pct=(bill.participantPercentages&&bill.participantPercentages[item.id])||'';return '<label class="check-option"><input type="checkbox" name="participant" value="'+item.id+'" '+((bill.participantIds||[]).indexOf(item.id)>=0?'checked':'')+'> '+escapeHtml(item.name)+' <input class="input participant-percentage" name="percentage-'+item.id+'" type="number" min="0" max="100" step="0.01" value="'+pct+'" placeholder="0"><span class="percentage-suffix">%</span></label>';}).join('');
  ...
  function renderRemoteDevices() {
    var pending=Array.isArray(remoteAccessStatus.pendingRequests)?remoteAccessStatus.pendingRequests:[];
    var devices=Array.isArray(remoteAccessStatus.devices)?remoteAccessStatus.devices:[];
    var pendingHtml=pending.length?pending.map(function(item){return '<div class="remote-device-row pending"><div><strong>'+escapeHtml(item.name||'Dispositivo')+'</strong><small>Aguardando sua autorização</small></div><div class="row wrap"><button class="lime-btn compact-btn" data-action="approve-pairing" data-request-id="'+escapeHtml(item.id)+'">Aprovar</button><button class="danger-btn compact-btn" data-action="deny-pairing" data-request-id="'+escapeHtml(item.id)+'">Negar</button></div></div>';}).join(''):'<p class="remote-service-note">Nenhuma solicitação pendente.</p>';
    var devicesHtml=devices.length?devices.map(function(item){var status=item.status||'active',label=status==='blocked'?'Bloqueado':status==='inactive'?'Inativo':'Ativo';var action=status==='active'?'<button class="secondary-btn compact-btn" data-action="set-device-status" data-device-id="'+escapeHtml(item.id)+'" data-device-status="inactive">Inativar</button><button class="danger-btn compact-btn" data-action="set-device-status" data-device-id="'+escapeHtml(item.id)+'" data-device-status="blocked">Bloquear</button>': '<button class="lime-btn compact-btn" data-action="set-device-status" data-device-id="'+escapeHtml(item.id)+'" data-device-status="active">Reativar</button>';return '<div class="remote-device-row '+status+'"><div><strong>'+escapeHtml(item.name||'Dispositivo')+'</strong><small>'+label+' · vinculado em '+escapeHtml(new Date(item.createdAt).toLocaleString('pt-BR'))+'</small></div><div class="row wrap">'+action+'</div></div>';}).join(''):'<p class="remote-service-note">Nenhum dispositivo autorizado.</p>';
    return '<div class="remote-devices-manager"><div class="remote-devices-section"><div class="settings-heading"><div><span class="settings-kicker">SOLICITAÇÕES</span><h3>Dispositivos aguardando aprovação</h3><p class="card-subtitle">Autorize somente aparelhos que você reconhece.</p></div></div>'+pendingHtml+'</div><div class="remote-devices-section"><div class="settings-heading"><div><span class="settings-kicker">DISPOSITIVOS VINCULADOS</span><h3>Aparelhos conectados</h3><p class="card-subtitle">Inative temporariamente ou bloqueie o acesso de um aparelho.</p></div></div>'+devicesHtml+'</div></div>';
  }
  ...
  function decidePairing(requestId, approved) {
    if(!isAdministrator(getActiveProfile())||!root.rbDesktop||!root.rbDesktop.sync.decidePairing)return;
    root.rbDesktop.sync.decidePairing(requestId,approved).then(function(status){applyRemoteAccessStatus(status);toast(approved?'Dispositivo aprovado.':'Solicitação negada.');}).catch(function(){toast('Não foi possível atualizar a solicitação.');});
  }
  function setDeviceStatus(deviceId, status) {
    if(!isAdministrator(getActiveProfile())||!root.rbDesktop||!root.rbDesktop.sync.setDeviceStatus)return;
    var label=status==='blocked'?'bloquear':status==='active'?'reativar':'inativar';
    confirmAction((label.charAt(0).toUpperCase()+label.slice(1))+' dispositivo','O aparelho perderá o acesso remoto até ser reativado.',function(){root.rbDesktop.sync.setDeviceStatus(deviceId,status).then(function(value){applyRemoteAccessStatus(value);toast('Dispositivo '+(status==='active'?'reativado':status==='blocked'?'bloqueado':'inativado')+'.');}).catch(function(){toast('Não foi possível alterar o dispositivo.');});});
  }
  ...
  function actionPermission(action) {
    action=String(action||'');
    if(!action||['settings-tab','copy-pairing-code','copy-browser-link','copy-remote-access-link','mobile-sidebar','toggle-sidebar','close-modal','cancel-profile-unlock','open-profiles','switch-profile','new-profile','edit-profile','delete-profile','open-module-report','open-loan-report','print-loan-report','loan-details','salary-loan-details','export-backup','toggle-all-loan-installments','open-inactive-accounts','open-inactive-cards','saving-history','investment-history','manage-investment-categories','home-bill-history','check-for-updates','force-update','open-permissions-popout','verify-update-modal','download-update-modal','pay-home-bill','approve-pairing','deny-pairing','set-device-status'].indexOf(action)>=0)return '';
  ...
  function handleClick(ev) {
    ...
    if (action === 'approve-pairing' || action === 'deny-pairing') return decidePairing(btn.getAttribute('data-request-id'), action === 'approve-pairing');
    if (action === 'set-device-status') return setDeviceStatus(btn.getAttribute('data-device-id'), btn.getAttribute('data-device-status'));
  ...
  function init() {
    ...
    function ensureResponsiveShell() { if (isMobileSidebar()) { $('app-shell').style.setProperty('grid-template-columns','0 minmax(0,1fr)','important'); $('app-shell').querySelector('.main').style.setProperty('grid-column','2','important'); } }
